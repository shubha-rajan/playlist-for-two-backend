...
from playlist import playlists, recommendations
...
class TestRecommendations(TestCase):
    ...
    @classmethod
    def setup_class(cls):
        cls.mock_refresh_token_patcher = patch(
            'playlist.recommendations.refresh_token')
        cls.mock_get_patcher = patch(
            'playlist.recommendations.requests.get')
        cls.mock_get_seeds_patcher = patch(
            'playlist.recommendations.get_seeds')
        cls.mock_get_seed_names_patcher = patch(
            'playlist.recommendations.get_seed_names')
        ...
        cls.intersection = {
            ...
            'common_artists': [
                {'name': "Afasi & Filthy", 'id': "0I2XqVXqHScXjHhk6AYYRe"},
                {'id': "0oSGxfWSnnOXhD2fKuz2Gy", 'name': 'David Bowie'},
                {'id': "1VBflYyxBhnDc9uVib98rw", 'name': 'Icona Pop'}
            ],
            'common_genres': ["swedish hip hop", "art rock", "glam rock", "permanent wave"],
        }

        cls.seeds = [{"glam rock": "genre"}, {"1VBflYyxBhnDc9uVib98rw": "artist"},
                     {"permanent wave": "genre"}, {"15iosIuxC3C53BgsM5Uggs": "song"}]
    ...
    def test_get_recommendations(self):
        ...
        self.mock_get_seed_names.return_value = ["glam rock (genre)",
                                                 "Icona Pop (artist)",
        ...
        result = {
            'seeds': ["glam rock (genre)",
                      "Icona Pop (artist)",
                      "permanent wave (genre)",
                      "All Night (song)"],
            'recommendations': [
                {
                    'artists': [{'id': '134GdR5tUtxJrf8cpsfpyY', 'name': 'Elliphant'},
                                {'id': '1D2oK3cJRq97OXDzu77BFR', 'name': 'Ras Fraser Jr.'}],
                    'explicit': False,
                    'id': '1TKYPzH66GwsqyJFKFkBHQ',
                    'name': 'Music Is Life'},
                {
        ...
        recs = recommendations.get_rec_from_intersection(
            self.intersection, False)

        self.assertDictEqual(result, recs)
    ...
    def test_explicit_filter(self):
        ...
        result = recommendations.get_rec_from_intersection(
