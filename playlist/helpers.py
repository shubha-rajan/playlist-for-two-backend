...
def get_seed_names(seeds):
    ...
    for seed in seeds:
        if 'song' in seed.values():
            ...
            response = requests.get(F'https://api.spotify.com/v1/tracks/{song_id}', headers={'Authorization': 'Bearer {}'.format(user.sp_access_token) 
            })
            song_name = json.loads(response.text)['name']
            seed_names.append(F'{song_name} (song)')
        elif 'artist' in seed.values():
            artist_id = list(seed.keys())[0] 
            response = requests.get(F'https://api.spotify.com/v1/artists/{artist_id}', headers={'Authorization': 'Bearer {}'.format(user.sp_access_token) 
            })
...
def generate_playlist(user1, user2):
    ...
    return {'seeds': seeds, 'uri': playlist_uri}
