...
def generate_playlist(user1, user2, filter_explicit, seeds=None, features=None, ):
    ...
    playlist_info = {
        ...
        'public':'false',
        'collaborative': 'false',
        'description': F"This playlist was generated by Spotify and the Playlist for Two app based on common songs, artists and genres in {user1.name} and {user2.name}'s music libraries. The seeds used to generate this playlist are {', '.join(seed_names)}"
    }
    ...
    tracks = ','.join(['spotify:track:{}'.format(track['id']) for track in recommendation_list])
    ...
    if add_tracks.status_code == 200 or add_tracks.status_code == 201:
        return {'seeds': seed_names, 'uri':F'spotify:playlist:{playlist_id}' , 'description': playlist_info}
    else:
